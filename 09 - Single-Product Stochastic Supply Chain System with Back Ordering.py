'''STOCHASTIC SINGLE PRODUCT SUPPLY CHAIN SYSTEM WITH BACK ORDERING'''
...
class DCOrderGenerator(object):
    ...
    def __init__(self,env,name=''):
        ...
        self.name = name
        self.demand=distribution(df2.iat[df2.loc[df2['Parameter']=='Demand'].index[0],1],df2.iat[df2.loc[df2['Parameter']=='Demand'].index[0],2],df2.iat[df2.loc[df2['Parameter']=='Demand'].index[0],3],df2.iat[df2.loc[df2['Parameter']=='Demand'].index[0],4])            
    ...
    def ordertoDC (self) :
        ...
        print('Time {1}: {0} places order of {2} units to DC'.format(self.name ,self.env.now,self.demand))
        processing_time=0
        ...
        if wait_time>0.1:
            P.total_backorder_cost+=wait_time*self.demand*P.unit_backorder_cost
